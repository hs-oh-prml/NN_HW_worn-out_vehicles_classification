...
config.SYSTEM.TEAM_NAME = 'baseline' # text.replace(" ","")
config.SYSTEM.SAVE_DIR = './save_csv/baseline' # 모델 파라미터가 저장되는 위치
config.SYSTEM.SAVE_CHECKPOINT = './checkpoint/baseline' # 모델 파라미터가 저장되는 위치
...
for epoch in range(1,num_epochs+1):
    ...
    if loss_valid < loss_min:
        loss_min = loss_valid
        torch.save(model.state_dict(), os.path.join(config.SYSTEM.SAVE_CHECKPOINT, './vehicle_recognition_best.pth')) 
...
with torch.no_grad():
    ...
    best_network = model
    best_network.load_state_dict(torch.load(os.path.join(config.SYSTEM.SAVE_CHECKPOINT, './vehicle_recognition_best.pth'))) 
