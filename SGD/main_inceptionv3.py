...
config.SYSTEM.TEAM_NAME = 'inception_v3' # text.replace(" ","")
...
config.TRAIN.MODEL = 'inception_v3'

# 사용할 augmentation
config.TRAIN.AUGMENTATION = {'size': 299, 'use_flip': True, 'use_color_jitter': True,
                             'use_gray_scale': True, 'use_normalize': True}  
config.TEST.AUGMENTATION = {'size': 299, 'use_normalize': True}
...
for epoch in range(1,num_epochs+1):
    ...
    for step , (images, targets) in enumerate(train_loader):
        ...
        loss_train_step = loss1 + 0.4*loss2
